...
void handleColour (TPacket *packet)
{
	...
	} else if (packet->params[0] == 1)
	{
		...
		printf("Colour Detected: NOT AN OBSTACLE\n");
	}			
}
...
void getParams(TPacket *commandPacket,char ch)
{
	...
	{
		commandPacket->params[0] = 5; commandPacket->params[1] = 100;
    }
	else
	{
		commandPacket->params[0] = 15; commandPacket->params[1] = 100;
	}
}
...
int main()
{
	...
	{
		char ch;
		printf("Command (f=forward, b=reverse, l=turn left, r=turn right, s=stop, c=clear stats, g=get_stats a = find_colour q=exit)\n");
		scanf("%c", &ch);
